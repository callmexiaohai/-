...
auto();
var targetstr="遍历(\"className\",\"id\",\"text\",0,0,0)";
var targetstrnew="";
var targetstrAll="";

//要显示的空间数据。
var show_ary = ["className", "depth", "indexInParent","id", "text","desc", "bounds",  "clickable", "scrollable"];
            ...
            for (let i = 0; i < 当前数组.length; i++) {
                ...
                if (uiObj) {
                    ...
                    canvas.drawRect(rect, paint1);
                        ...
                        for (let i = 0; i < show_ary.length; i++) {
                            if(targetstrAll=="className"){
                                targetstrAll=show_ary[i]+"(\""+uiObj[show_ary[i]]()+"\")";                            
                            }else{
                                targetstrAll=targetstrAll+"."+show_ary[i]+"(\""+uiObj[show_ary[i]]()+"\")";
                            }
                               
                                
                            canvas.drawText(show_ary[i] + ": " + uiObj[show_ary[i]](), w / 10, h / 5 + paint1.getTextSize() * i, paint1);
                            // 提取信息
                            log(show_ary[i] + ": " + uiObj[show_ary[i]]() );
                            if (show_ary[i]=="className") {
                                if(uiObj[show_ary[i]]()!=null){
                                    targetstr=targetstr.replace(/className/,uiObj[show_ary[i]]());
                                     }else{
                                    targetstr=targetstr.replace(/"className"/,"null");
                                }
                            }
                            if (show_ary[i]=="id") {
                                if(uiObj[show_ary[i]]()!=null){
                                    targetstr=targetstr.replace(/id/,uiObj[show_ary[i]]());
                                }else{
                                    targetstr=targetstr.replace(/"id"/,"null");
                                }
                            }
                            if (show_ary[i]=="text") {
                                if(uiObj[show_ary[i]]()!=null){
                                    targetstr=targetstr.replace(/text/,uiObj[show_ary[i]]());
                                }else{
                                    targetstr=targetstr.replace(/"text"/,"null");
                                }
                            }                             
                        ...
                        log("复制内容："+targetstrnew);
                        log(targetstrAll+".findOnce()")
                        targetstr="遍历(\"className\",\"id\",\"text\",0,0,0)";
                        // targetstrAll="";
                };
            };
...
win_.but_g.longClick(function(){
    log(targetstrnew)
    // setClip(targetstrnew)
    setClip(targetstrAll)
    toastLog("复制成功");
    toastLog("已关闭");
    exit();
});
