 ...
 * @param {倒计时时间} 随机间隔时间 
 */
人工划屏.间隔倒计时=function(随机间隔时间,strsmsg,logs){
    // log(随机间隔时间,strsmsg)
 ...
 * @param {滑动的方位} strs 
 */
人工划屏.划 = function(strs,timesInterval,logs){
    if(strs=="上"){
        // log('进入了上滑的位置',logs)
        ...
        this.间隔倒计时(timesInterval,'上',logs);
    }
    if(strs=="下"){
        ...
        this.sml_move(x1,y1,x2,y2,random(500,700));
        this.间隔倒计时(random(timesInterval, timesInterval+3),'下',logs);
    }
    ...
    if(strs=="左"){
        ...
        this.sml_move(x1,y1,x2,y2,random(500,700));
        this.间隔倒计时(random(timesInterval, timesInterval+3),'左',logs);
    }
    if(strs=="右"){
        x1=random(device.width*0.1,device.width*0.2);
        y1=random(device.height*0.6,device.height*0.7);
        x2=random(device.width*0.8,device.width*0.9);
        y2=random(device.height*0.5,device.height*0.6);
        this.sml_move(x1,y1,x2,y2,random(500,700));
        this.间隔倒计时(random(timesInterval, timesInterval+3),'右',logs);
    }
    if(strs=="左窗口"){
        ...
        y2=random(device.height*0.5,device.height*0.6);
        this.sml_move(x1,y1,x2,y2,random(500,700));
        this.间隔倒计时(random(timesInterval, timesInterval+3),'左窗口划屏操作',logs);
    }
}
